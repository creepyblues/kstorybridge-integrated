...
/**
 * Fixed embedding generation script that works around Supabase vector type issues
 * Uses raw SQL to verify vector storage since Supabase JS client has vector compatibility issues
 */
...
async function storeTitleEmbeddings(titleId, embeddings) {
  ...
  try {
    ...
    if (updateError) {
      console.error('❌ Database error storing embeddings:', updateError);
    ...
    // Verify using vector search function instead of direct retrieval
    // This works around the Supabase JS vector type issue
    console.log('🔍 Verifying storage using vector search...');
    
    const { data: searchResults, error: searchError } = await supabase.rpc('match_titles_by_embedding', {
      query_embedding: embeddings.combined_embedding,
      match_threshold: 0.9, // Very high threshold to find exact match
      match_count: 1
    });

    if (searchError) {
      console.error('❌ Vector search verification failed:', searchError.message);
      return false;
    }

    // Check if we found the title we just updated
    const foundTitle = searchResults?.find(result => result.title_id === titleId);
    
    if (foundTitle && foundTitle.similarity > 0.99) {
      console.log(`✅ Embedding verified through vector search!`);
      console.log(`   Found title: ${foundTitle.title_name_en || foundTitle.title_name_kr}`);
      console.log(`   Similarity: ${foundTitle.similarity.toFixed(6)} (should be ~1.0)`);
      return true;
    } else {
      console.error('❌ Embedding verification failed - not found in vector search');
      console.error(`   Search results: ${searchResults?.length || 0} results`);
      return false;
    }
...
async function main() {
  console.log('🚀 Starting FIXED embedding generation process...');
  console.log('🔧 Using vector search verification to work around Supabase JS vector type issue');
  ...
  try {
    ...
    for (const title of titles) {
      try {
        ...
        if (!isDryRun && processed + failed < titles.length) {
    ...
    console.log('\n🎉 FIXED embedding generation complete!');
    console.log('🔧 Used vector search verification to work around Supabase JS vector type issue');
    
    if (!isDryRun && processed > 0) {
      console.log('\n🔍 Next steps:');
      console.log('1. Test OpenAI chatbot with real queries');
      console.log('2. Generate more embeddings: run this script with higher --limit');
      console.log('3. Vector search should now work properly!');
    }
