...
- [x] `vercel.json` exists in `apps/creator/`
- [x] `.vercelignore` exists in `apps/creator/`
...
```bash
git checkout -b creator-staging
git push origin creator-staging
```

### Configure in Vercel
1. Vercel Dashboard → Project Settings → Git
2. Enable automatic deployments for `creator-staging` branch
3. Staging URL will be: `creator-staging-*.vercel.app`
