  ...
  const handleScrapeData = async () => {
    ...
    try {
      ...
      if (mockResult.success && mockResult.data) {
        ...
        toast({ title: "Success", description: `Data scraped successfully! Populated ${mockResult.extractedFields.length} fields with ${Math.round(mockResult.confidence * 100)}% confidence` });
      } else {
        toast({ title: "Error", description: mockResult.error || "Failed to scrape data from URL"});
      }
    } catch (error) {
      console.error("Scraping error:", error);
      toast({ title: "Error", description: "Failed to scrape data. Please try again or enter manually."});
    } finally {
