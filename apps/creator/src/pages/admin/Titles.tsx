...
export default function AdminTitles() {
  ...
  const loadData = async (force = false) => {
    try {
      ...
      toast({ title: "Error", description: "Error loading titles"});
    } finally {
      setLoading(false);
    }
  };
    ...
    <AdminLayout>
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto py-4 sm:py-6 lg:py-8 px-3 sm:px-6 lg:px-8">
            ...
            <div className="mb-8 sm:mb-12">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                {featuredTitles.map((featured) => {
                  ...
                  return (
                    <Link key={featured.id} to={`/titles/${title.title_id}`} className="block group">
                      <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:scale-105">
                        ...
                        <div className="p-3">
                          <h3 className="text-sm font-semibold text-gray-800 mb-1 line-clamp-2 group-hover:text-hanok-teal transition-colors">
                          ...
                          {title.genre && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {(Array.isArray(title.genre) ? title.genre.slice(0, 1) : [title.genre]).map((g, idx) => (
                                <span key={idx} className="inline-block bg-cyan-100 text-cyan-700 px-2 py-0.5 rounded-full text-xs font-medium">
                                  {formatGenre(g)}
                                </span>
                              ))}
          ...
          {/* Divider */}
          {featuredTitles.length > 0 && <div className="border-t border-gray-200 my-8 sm:my-12"></div>}
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-4 sm:gap-0">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
                ...
                <Link to="/titles/new">
                  <Button className="bg-hanok-teal hover:bg-hanok-teal-600 text-white px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base rounded-lg font-medium w-full sm:w-auto">
