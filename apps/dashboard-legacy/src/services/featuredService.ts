...
export const featuredService = {
    ...
    try {
      ...
      const mainQueryPromise = withRetry(
        ...
        },
        {
          maxRetries: 1,
          baseDelay: 1000,
          timeoutMs: 60000, // Match our new Supabase client timeout settings
  ...
  async getFallbackFeaturedTitles(): Promise<FeaturedWithTitle[]> {
    try {
      ...
      const fallbackTimeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(() => {
          console.warn('⏰ [VERBOSE] Fallback query timeout (60s)');
          reject(new Error('Fallback query timeout'));
        }, 60000)
      );

      console.log('🎬 [VERBOSE] Executing fallback query with 60s timeout...');
  ...
  async addFeaturedTitle(titleId: string, note?: string): Promise<void> {
    const { error } = await withRetry(
      ...
      {
        maxRetries: 2,
        operationName: 'addFeaturedTitle',
        retryCondition: isNetworkError
      }
    );
  ...
  async removeFeaturedTitle(featuredId: string): Promise<void> {
    const { error } = await withRetry(
      ...
      {
        maxRetries: 2,
        operationName: 'removeFeaturedTitle',
        retryCondition: isNetworkError
      }
    );
  ...
  async updateFeaturedNote(featuredId: string, note: string): Promise<void> {
    const { error } = await withRetry(
      ...
      {
        maxRetries: 2,
        operationName: 'updateFeaturedNote',
        retryCondition: isNetworkError
      }
    );
