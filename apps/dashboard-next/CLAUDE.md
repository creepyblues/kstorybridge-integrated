...
**Production URL**: https://dashboard-next.kstorybridge.com (TBD)
...
### From this directory (`apps/dashboard-next/`)
```bash
...
```bash
npm run dev:dashboard-next # Alternative way to start dev server
...
### Why Dashboard Next Exists
V1 (apps/dashboard) had 279 files with mixed buyer/creator logic, complex fallbacks, and tight coupling. Dashboard Next is a clean rebuild with 82% fewer files (~50 total), buyer-only focus, and simpler patterns.

### Shared Supabase Database
All apps (dashboard, dashboard-next, creator, website) share the same Supabase project. This means:
