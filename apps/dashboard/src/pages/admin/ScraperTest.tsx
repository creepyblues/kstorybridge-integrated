...
import { Badge, Button, Input, Label, Card, CardContent, CardHeader, CardTitle, Separator, Textarea } from "@kstorybridge/ui";
import {
  AlertCircle,
  CheckCircle,
  Globe,
...
export default function AdminScraperTest() {
  ...
  const handleScrape = async () => {
    if (!url.trim()) {
      toast({ title: "Error", description: "Please enter a URL to scrape"});
      return;
    }
    ...
    try {
      ...
      if (scrapingResult.success && scrapingResult.data) {
        ...
        toast({ title: "Success", description: `Scraping completed! Extracted ${scrapingResult.extractedFields.length} fields with ${Math.round(scrapingResult.confidence * 100)}% confidence` });
      } else {
        toast({ title: "Error", description: scrapingResult.error || "Scraping failed"});
      }
    } catch (error) {
      ...
      toast({ title: "Error", description: "Scraping failed: " + errorMessage});
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <PageContainer>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-2xl font-bold text-gray-800">Scraper Test Lab</h1>
                  ...
                  <div className="mt-4 pt-4 border-t space-y-2">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (health) {
                          toast({ title: "Success", description: `Backend healthy: ${health.service}` });
                        } else {
                          toast({ title: "Error", description: 'Backend unavailable - make sure server is running'});
                        }
                      }}
                  ...
                  <div className="pt-4 border-t space-y-2">
                    <Button
                      onClick={async () => {
                        try {
                          ...
                          if (response.ok) {
                            toast({ title: "Success", description: "✅ Feedback submitted! This will improve scraper accuracy."});
                          } else {
                            throw new Error('Failed to submit feedback');
                          }
                        } catch (error) {
                          console.error('Feedback submission failed:', error);
                          toast({ title: "Error", description: "❌ Could not submit feedback. Data logged to console."});
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (response.ok) {
                            ...
                            toast({ title: "Success", description: `🧪 Test case created: ${data.testId}` });
                          } else {
                            throw new Error('Failed to create test case');
                          }
                        } catch (error) {
                          console.error('Test case creation failed:', error);
                          toast({ title: "Error", description: "❌ Could not create test case"});
                        }
                      }}
                      variant="outline"
                    ...
                    <Button
                      onClick={() => {
                        ...
                        toast({ title: "Success", description: "Data validated! Check console for details."});
                      }}
                      variant="outline"
                      className="w-full"
                    >
