...
export default function AdminUserApproval() {
  ...
  const loadUsers = async () => {
    try {
      ...
      toast({ title: "Info", description: 'Using placeholder emails. Run the database migration for real email addresses.' });
  ...
  const saveUserTier = async (userId: string) => {
    ...
    try {
      ...
      toast({ title: "Error", description: 'Failed to update user tier' });
    } finally {
      setSavingUsers(prev => {
    ...
    try {
      ...
      try {
        ...
        if (emailError) {
          ...
          toast({ title: "Error", description: `User approved but email failed: ${emailError.message || 'Unknown error'}` });
        } else {
          console.log('Email function response:', data);
          
          if (data?.success) {
            toast({ title: "Success", description: `✅ User approved and email sent via Resend! (ID: ${data.emailId?.substring(0, 8) || 'N/A'})` });
          } else {
            console.error('Email function error:', data);
            toast({ title: "Error", description: `❌ User approved but email failed: ${data.message || 'Unknown error'}` });
          }
        }
      } catch (error) {
        console.error('Failed to invoke email function:', error);
        toast({ title: "Error", description: `User approved but email failed: ${error.message || 'Unknown error'}` });
      }
