...
import { FeaturePageLayout } from '@/components/features/shared';
import FormatSpotlightCard from '@/components/format-spotlight/FormatSpotlightCard';
import {
  FORMAT_DISPLAY_NAMES,
  FORMAT_ICONS,
  FORMAT_DESCRIPTIONS,
  type FormatType,
  getFormatSpotlightData,
} from '@/services/formatSpotlightService';
import { getDashboardUrl } from '@/config/urls';

const VALID_FORMATS: FormatType[] = ['film', 'tv_series', 'animation', 'microdrama', 'audio_drama'];

export default function FormatSpotlightPage() {
  ...
  const handleCardClick = (titleId: string) => {
    // Redirect to dashboard title detail — ProtectedRoute will save the
    // intended path and redirect back after login/signup
    window.location.href = `${dashboardUrl}/buyers/titles/${titleId}`;
  };

  return (
    <FeaturePageLayout>
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-6">
        {/* Header */}
        ...
        {!isLoading && !error && data && data.length > 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 text-sm mb-3">
              Sign up to explore full title details, pitch decks, and more.
            </p>
            <button
              onClick={() => { window.location.href = `${dashboardUrl}/signup`; }}
              className="inline-flex items-center gap-2 px-6 py-3 bg-hanok-teal text-white rounded-full font-medium hover:bg-hanok-teal/90 transition-colors"
            >
              Get Started Free
