-- Update the user routing function to explicitly handle the plan field
-- Even though it has a default, it's better to be explicit
...
BEGIN
  ...
  IF NEW.raw_user_meta_data->>'account_type' = 'buyer' THEN
    ...
    VALUES (
      ...
      COALESCE((NEW.raw_user_meta_data->>'plan')::buyer_plan, 'free'::buyer_plan)
    );
