...
- Introduction names remain reserved until an authoritative current workflow exists. The legacy production `request` table contains one historical contact row but no status/completion boundary and is not a canonical source; see [ANALYTICS_INTRODUCTION_SOURCE_AUDIT_2026-07-13.md](ANALYTICS_INTRODUCTION_SOURCE_AUDIT_2026-07-13.md). Approval and publication have locally validated authoritative implementations, but must not be reported as live until the coordinated production cutover passes.
