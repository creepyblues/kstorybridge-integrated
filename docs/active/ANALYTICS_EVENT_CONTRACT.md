...
- Product-engagement names are implemented and tested on `v2` under `AR-206`. The scheduled report and tracked analytics skill now query only canonical authenticated names under `AR-400`, keep public-trial events separate, and require a full-window live-at boundary; production release remains pending.
