...
| Repository progress audit | Monday 15:00 UTC via `.github/workflows/analytics-progress.yml` | Parse this program's `AR-*` checklist; best-effort probes verify `www`, default-branch workflow, release-PR CI, the clean-production observation window, GA filter evidence, and the privacy-safe scheduled-delivery streak | Workflow exists on `v2`; default-branch activation is pending `AR-014`, GitHub Actions is account-billing locked under `AR-016`, the complete clean-filter cutover is unset, and the production delivery ledger is not yet live. The checks are read-only and never rerun, merge, deploy, or mutate CI/GA. |
| Local progress fallback | Monday 08:05 America/Los_Angeles via user crontab | Maintain delivery and external-gate monitoring until the repository schedule is active | Installed and verified; resolves the local Analytics read credential in memory for `AR-106` and automatically stops sending when the plan marker becomes `DONE`. |
...
- A legacy `request` table contains one historical contact row but has no current active write boundary, status, or completion timestamp; there is still no authoritative introduction workflow, buyer-approval timestamp, or interest-transition timestamp.
