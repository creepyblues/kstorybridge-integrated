...
`AR-205` partial implementation evidence: buyer `interest_submitted` now fires only after `express-interest` confirms its Supabase upsert. It replaces legacy `title_interest_submitted` and removes the title name, note-presence flag, and client timestamp. A privacy-focused event test and the full dashboard build pass. The task remains open because introductions lack an authoritative record and subscription outcomes still require webhook-side instrumentation.
...
`AR-207` partial evidence is maintained in [ANALYTICS_EVENT_TEST_MATRIX.md](ANALYTICS_EVENT_TEST_MATRIX.md). Website trusted-email engagement, dashboard email auth, creator email auth, buyer interest, and both creator title-entry paths now prove successful outcomes fire once and failure/duplicate/untrusted paths emit no success. The task remains open for OAuth/profile boundaries, page-level buyer tool boundaries, and reserved commercial/admin outcomes that are not implemented yet.
...
| 2026-07-13 | Added the critical-event test matrix and closed boundary-test gaps for email engagement, dashboard email auth, buyer interest, creator email auth, and creator title submission. | New integration tests prove success emits once while failed, duplicate, page-load-only, and untrusted paths emit no successful outcome; the matrix explicitly keeps OAuth, buyer-tool page boundaries, and unimplemented reserved outcomes open. | Add OAuth/profile and buyer-tool page boundary coverage, then implement and test the remaining reserved outcomes before closing `AR-207`. |
