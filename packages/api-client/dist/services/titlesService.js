...
export const titlesService = {
    ...
    async getTitlesByCreator(creatorId) {
        const { data, error } = await supabase
            .from("titles")
            .select("*")
    ...
    async getTitlesByCreatorRights(userId) {
        try {
            const { data, error } = await supabase
                .from("titles")
                .select("*")
    ...
    async getTitlesWithPitches(limit = 6) {
        const { data, error } = await supabase
            .from('titles')
            .select('*')
            .not('pitch', 'is', null)
    ...
    async searchTitles(query, filters) {
        ...
        if (filters?.content_format) {
            queryBuilder = queryBuilder.eq("content_format", filters.content_format);
        }
