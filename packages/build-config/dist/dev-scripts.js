...
export class DevScripts {
    ...
    getPackagesInBuildOrder() {
        const packages = this.getPackages();
        // Simple ordering: packages first, then apps
        const packageOrder = packages.sort((a, b) => {
            if (a.startsWith('packages/') && b.startsWith('apps/'))
                return -1;
            if (a.startsWith('apps/') && b.startsWith('packages/'))
                return 1;
    ...
    hasScript(packagePath, scriptName) {
        try {
            const packageJson = JSON.parse(readFileSync(path.join(packagePath, 'package.json'), 'utf-8'));
