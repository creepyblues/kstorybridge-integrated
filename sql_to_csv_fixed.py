#!/usr/bin/env python3
"""
Fixed SQL to CSV Converter
Handles real-world SQL backup files with proper column mapping.

Usage:
    python sql_to_csv_fixed.py input.sql output.csv
    python sql_to_csv_fixed.py input.sql --output-dir ./csv_files
    python sql_to_csv_fixed.py input.sql --table users --output users.csv
"""
...
class FixedSQLToCSVConverter:
    ...
    def parse_sql_file(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for table_name, table_info in self.tables.items():
            if table_name in self.insert_statements:
                columns = table_info.get('columns', [])
                values = self.insert_statements[table_name]['values']
                ...
                for value_row in values:
                    ...
                    for i, value in enumerate(value_row):
                        if i < len(columns):
                            ...
                            # Use generic column name for extra values
...
def main():
    ...
    converter = FixedSQLToCSVConverter(args.sql_file)
